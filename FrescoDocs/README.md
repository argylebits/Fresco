# Fresco

**Daily AI-generated images for any project.**

Fresco is a CLI tool that generates a fresh image every day using the Google Gemini Imagen API and publishes it to Cloudflare R2. Drop the stable URL into any README, website, or app — it updates itself.

![Daily Fresco](https://pub-placeholder.r2.dev/fresco/today.jpg)

---

## How it works

```
fresco generate
  → reads config from environment variables
  → calls Gemini Imagen API
  → uploads to Cloudflare R2
  → updates gallery.md
```

Your project just points at a stable URL:

```markdown
![Daily Image](https://pub-xxxx.r2.dev/your-project/today.jpg)
```

That image changes every day. You never touch it again.

---

## Install

```bash
brew install argylebits/fresco
```

---

## Quick start

```bash
# Set up a project for daily images
cd your-project
fresco init

# Generate your first image now
fresco generate
```

`fresco init` will:
- Walk you through configuring your prompt and credentials
- Create `.env` with your configuration (gitignored)
- Write `.github/workflows/fresco.yml` for daily automation
- Add the image URL to your README
- Generate your first image immediately

---

## The Fresco project uses itself

The banner image at the top of this README is generated by Fresco daily.
Browse the [Gallery](gallery.md) for the full image history.

---

## Documentation

| Document | |
|---|---|
| [Vision & Roadmap](docs/VISION.md) | What Fresco is, where it's going, the three phases |
| [CLI Reference](docs/CLI.md) | Every command, flag, and environment variable |
| [Prompt Configuration](docs/PROMPT_CONFIG.md) | How to write great prompts |
| [Architecture](docs/ARCHITECTURE.md) | How it all fits together, per phase |
| [Development Guide](AGENTS/DEVELOPMENT.md) | Local setup, testing, contributing |
| [Deployment Guide](docs/DEPLOYMENT.md) | Self-hosting the Fresco server (Phase 2) |

---

## Phases

Fresco is being built in three phases:

**Phase 1 — CLI** ← you are here
Local tool. GitHub Actions for scheduling. Gemini + R2 direct.

**Phase 2 — Self-hosted server**
Persistent scheduler for non-GitHub projects. Run your own Fresco server.

**Phase 3 — Hosted service**
Sign up and use Fresco without running any infrastructure.

The CLI works the same way in all three phases. Phase 2 and 3 just change what's behind it.

---

## License

MIT
